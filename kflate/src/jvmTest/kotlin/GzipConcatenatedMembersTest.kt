...
class GzipConcatenatedMembersTest {
    ...
    @Test
    fun `GzipDecompression decompress should handle single member (backward compatibility)`() {
        val originalData = "Hello KFlate World!".encodeToByteArray()
        val compressed = KFlate.compress(originalData, GzipCompression())

        val decompressed = KFlate.decompress(compressed, GzipDecompression())
        assertEquals(originalData.toList(), decompressed.toList())
    }

    @Test
    fun `GzipDecompression decompress should concatenate two members`() {
        val data1 = "Hello ".encodeToByteArray()
        val data2 = "World!".encodeToByteArray()

        val compressed1 = KFlate.compress(data1, GzipCompression())
        val compressed2 = KFlate.compress(data2, GzipCompression())
        ...
        val decompressed = KFlate.decompress(concatenated, GzipDecompression())
        val expected = data1 + data2
        assertEquals(expected.toList(), decompressed.toList())
    }

    @Test
    fun `GzipDecompression decompress should concatenate three members`() {
        ...
        val compressed1 = KFlate.compress(data1, GzipCompression())
        val compressed2 = KFlate.compress(data2, GzipCompression())
        val compressed3 = KFlate.compress(data3, GzipCompression())
    ...
    @Test
    fun `GzipDecompression decompress should reject trailing garbage`() {
        val originalData = "Hello KFlate World!".encodeToByteArray()
        val compressed = KFlate.compress(originalData, GzipCompression())
        ...
        try {
            KFlate.decompress(withGarbage, GzipDecompression())
    ...
    @Test
    fun `GzipDecompression decompress should reject incomplete member`() {
        val originalData = "Hello KFlate World!".encodeToByteArray()
        val compressed = KFlate.compress(originalData, GzipCompression())
        ...
        try {
            KFlate.decompress(concatenated, GzipDecompression())
    ...
    @Test
    fun `GzipDecompression decompress should handle empty member concatenation`() {
    ...
    @Test
    fun `GzipDecompression decompress should reject invalid magic bytes in second member`() {
        val data1 = "First ".encodeToByteArray()
        val compressed1 = KFlate.compress(data1, GzipCompression())
    ...
    @Test
    fun `GzipDecompression decompress should preserve order of concatenated members`() {
    ...
    @Test
    fun `GzipDecompression decompress should handle large concatenated members`() {
